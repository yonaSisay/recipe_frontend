...
import getRecipe from "@/graphql/query/getMyRecipe.gql";
import deleteRecipe from "@/graphql/mutations/deleteRecipe.gql";
import useMutation from "@/composables/authMutation.js";
	...
	<div class="h-full px-8 py-8">
		...
		>
			<h3
				...
				class="block text-gray-700 dark:text-white text-3xl font-bold mb-2"
			>
				Oops you dont have any recipes!
			</h3>
		</div>
		<div v-else class="relative overflow-x-auto mt-8">
			<table class="w-full text-sm text-left text-gray-500 dark:text-white">
				...
				<tbody>
					<tr
						class="bg-white dark:bg-[#5C5470] rounded-md border-b"
					...
					>
						<td
							...
							@click="toRecipe(recipe?.id)"
						>
							<img
								:src="recipe.coverImage"
								alt="Cover Image"
								class="w-8 h-8 object-cover rounded-lg mr-2"
							/>{{ recipe.title }}
						</td>
						<td scope="row" class="px-4 py-2">
							{{ new Date(recipe?.created_at).toLocaleDateString() }}
						</td>
						<td scope="row" class="px-4 py-2">{{ recipe.duration }}</td>
						<td scope="row" class="px-4 py-2">
							{{ recipe?.ratings_aggregate?.aggregate?.avg?.rating || 0 }}
						</td>
						<td scope="row" class="px-4 py-2">
							{{ recipe?.likes_aggregate?.aggregate?.count }}
						</td>
						<td scope="row" class="px-4 py-2" @click="toEditRecipe(recipe.id)">
							<svg
							...
							>
								...
								<g id="SVGRepo_iconCarrier">
									<path
										...
										:stroke="
											colorMode.preference === 'light' ? '#FF7008' : '#B9B4C7'
										"
										stroke-width="1.5"
										stroke-linecap="round"
										stroke-linejoin="round"
